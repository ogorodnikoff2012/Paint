...
PaintWorkspace *MainWindow::newFile(int width, int height, bool transparent)
{
    WorkspaceShell *s = new WorkspaceShell(0, width, height, transparent);
    area->addSubWindow(s);
    s->show();
...
void MainWindow::enableUndoButtons()
{
    if(getCurrentWorkspace())
    {
        aUndo->setEnabled(true);
        aRedo->setEnabled(true);
    }
    else
    {
        aUndo->setEnabled(false);
...
void MainWindow::createMenuBar()
{
    ...
    connect(mEdit, SIGNAL(aboutToHide()), SLOT(enableUndoButtons()));
...
{
    WorkspaceShell *s = new WorkspaceShell(0, 0, 0, false, filename);
    area->addSubWindow(s);
    s->show();
}
